...
public class NicknameBlocker {
    ...
    public void reload() {
        ...
        exactBlock.addAll(plugin.getConfigManager().getStringList("modules.bot-protection.nick-engelleme.tam-eslesme"));
        prefixBlock.addAll(plugin.getConfigManager().getStringList("modules.bot-protection.nick-engelleme.prefix-listesi"));
        suffixBlock.addAll(plugin.getConfigManager().getStringList("modules.bot-protection.nick-engelleme.suffix-listesi"));
        
        List<String> regexes = plugin.getConfigManager().getStringList("modules.bot-protection.nick-engelleme.regex-listesi");
        ...
        minLength = plugin.getConfigManager().getInt("modules.bot-protection.nick-engelleme.minimum-uzunluk", 3);
        maxLength = plugin.getConfigManager().getInt("modules.bot-protection.nick-engelleme.maksimum-uzunluk", 16);
        blockNumbersOnly = plugin.getConfigManager().getBoolean("modules.bot-protection.nick-engelleme.sadece-sayi-engel", true);
        blockSpecialChars = plugin.getConfigManager().getBoolean("modules.bot-protection.nick-engelleme.ozel-karakter-engel", true);
        capsThreshold = plugin.getConfigManager().getInt("modules.bot-protection.nick-engelleme.buyuk-harf-esik", 10);
    }

    public NicknameCheckResult check(String username) {
        if (!plugin.getConfigManager().getBoolean("modules.bot-protection.nick-engelleme.aktif", true)) {
